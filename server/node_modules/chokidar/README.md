...
>Chowkidar is a transliteration of a Hindi word meaning 'watchman, gatekeeper', ?????????????????????. This ultimately comes from Sanskrit _ ??????????????????_ (crossway, quadrangle, consisting-of-four).
